...
internal static class LateenBraceAttachmentPatch
{
    ...
    private static void Postfix(Mast __instance)
    {
        ...
        for (int i = 0; i < __instance.sails.Count; i++)
        {
            ...
            GPButtonRopeWinch winch = rig != null && rig.Brace != null
                ? FindAttachedWinch(__instance, sail.mastOrder, rig.Brace)
                : null;
            ...
            if (winch == null)
            {
                if (MissingWinchWarnings.Add(sail.GetInstanceID()))
                {
                    Plugin.Log?.LogWarning(
                        $"No unused winch is available for the brace on {sail.name}.");
                }
            ...
            if (!rig.Initialize(__instance, sail, connections) || rig.Brace == null)
                continue;

            winch.AttachToController(rig.Brace);
            winch.description = BraceWinchName;
            winch.gameObject.name = BraceWinchName;
...
internal static class LateenWinchLockPatch
{
        ...
        ref float ___currentInput)
    {
        ...
        bool canOperate = binding.Role == LateenControlRole.Brace
            ? binding.Rig.CanOperateBrace
