...
internal sealed partial class LateenYardRig
{
    ...
    private readonly List<RopeEffect> lowerBraceRopes =
    ...
    private void CreateLowerBraceRoute()
    {
        ...
        var fairleadObject = new GameObject("lateen lower-brace fairlead");
        fairleadObject.SetActive(false);
        lowerBraceFairlead = fairleadObject.transform;
        ...
        if (fairleadToHeel != null)
            lowerBraceRopes.Add(fairleadToHeel);

        fairleadObject.SetActive(true);

        var controllerObject = new GameObject($"{name} lower-brace controller");
        ...
        if (winchToFairlead != null)
            lowerBraceRopes.Add(winchToFairlead);

        LowerBrace = controllerObject.AddComponent<LateenLowerBraceController>();
    ...
    private void RefreshFairleadPosition()
    {
        ...
        lowerBraceFairlead.SetParent(parent, true);

        if (sheetFairlead == null)
            return;

        lowerBraceFairlead.position = sheetFairlead.position;
        lowerBraceFairlead.rotation = sheetFairlead.rotation;
    }
    ...
    private void UpdateRopeTension(float lowerBracePull)
    {
        ...
        {
            RopeEffect rope = lowerBraceRopes[i];
            if (rope == null)
                continue;
    ...
    {
        if (LowerBrace != null)
            Destroy(LowerBrace.gameObject);

        if (lowerBraceFairlead != null)
            Destroy(lowerBraceFairlead.gameObject);
    }
}
