...
internal sealed partial class LateenYardRig
{
    ...
    {
        var pivotObject = new GameObject("lateen lower-brace pivot");
        ...
        Vector2 ringTranslation)
    {
        ...
        for (int i = 0; i < yardContactSurfacePointsLocal.Length; i++)
        {
            ...
            Vector3 mastCenterWorld = GetCapsuleCenterlinePoint(
                mastContactCollider,
                pointWorld);

            clearance = Mathf.Min(
                clearance,
                (pointWorld - mastCenterWorld).magnitude - mastRadius);
        }
        ...
        out CapsuleCollider selectedCollider)
    {
        ...
        for (int i = 0; i < mast.mastCols.Length; i++)
        {
            ...
            Vector3 candidate = GetCapsuleCenterlinePoint(
                candidateCollider,
                contactWorld);
    ...
    private static Vector3 GetCapsuleCenterlinePoint(
        ...
        Vector3 contactWorld)
    {
        ...
        float distance = Mathf.Clamp(
            Vector3.Dot(contactWorld - centerWorld, axisWorld),
            -halfSegment,
            halfSegment);
